...
gene.sc <- readRDS(here("data", "bulk", "biomaRt_single-cell_2021-04-21.rda"))
...
cell.types[3] <- "extravillous"

map(cell.types, ~ correlations(data = joint, string = .x))
```

```{r}
string <- "Cytotrophoblast"
pair <- joint %>% dplyr::select(contains(string))

names(pair[,1])

ggplot(joint) + aes(x = RNA.Cytotrophoblast, y = bulk.cytotrophoblast, label = external_gene_name) + geom_point(alpha = .3) + xlab("Single-cell Expression") + ylab("Bulk Expression") + ggtitle(string)


ggplot(joint) + aes(x = RNA.Cytotrophoblast, y = bulk.cytotrophoblast, label = external_gene_name) + geom_point() +
  coord_cartesian(xlim = c(0, 50), ylim = c(0, 15))
```
