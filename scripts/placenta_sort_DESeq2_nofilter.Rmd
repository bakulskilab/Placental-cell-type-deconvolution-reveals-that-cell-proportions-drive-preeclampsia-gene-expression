...
make_gsea_input <- function(res, title) {
  ...
  print(ggplot(dup.stats.counts, aes(x = stat)) + geom_density() + ggtitle(paste0("Duplicate Test Statistics - ", title)) +
  ...
  #write_tsv(return, here("results", "GSEA", "sorted", paste0(title, "_dup_gsea_input_stats_", Sys.Date(), ".rnk")), col_names = FALSE)
...
No significant results with gprofiler2.
```{r, eval = F}
