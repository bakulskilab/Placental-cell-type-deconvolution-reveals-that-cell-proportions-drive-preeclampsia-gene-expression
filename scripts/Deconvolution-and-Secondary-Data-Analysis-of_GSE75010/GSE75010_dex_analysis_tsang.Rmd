...
meta <- readRDS(paste0(here("data", "GSE75010_with_abundances_2022-05-01.rda")))
...
Custom function to run limma model, print results, and output Volcano plot and model results for further analysis. https://support.bioconductor.org/p/98367/ brief discussion of logFC cutoffs

Limma paper:
"When one has a particular cut-off for log-fold-change in mind, the treat function can be used to test whether the log2-fold-change is greater than a threshold rather than merely different to zero (45). This can be effective for prioritizing results that are biologically as well as statistically significant."
...
run_limma_model <-
  function(d,
    ...
    hits$sig <- (hits$adj.P.Val < threshold) & (abs(hits$logFC) > logFC.cutoff)
    print(paste0(
    ...
    hits$sig <- factor(hits$sig,
                    levels = c(FALSE, TRUE),
                    labels = c("Does not meet p-value and logFC criteria", paste0("FDR-adjusted p-value < ", threshold, " & log2FC > ", logFC.cutoff)))
    ...
    plot <- ggplot(res) +
      ...
      geom_hline(yintercept = -log10(threshold),
                 linetype = "dotted") +
      geom_vline(xintercept = logFC.cutoff,
                 linetype = "dotted") +
            geom_vline(xintercept = -logFC.cutoff,
                 linetype = "dotted") +
...
```{r}
pca <-readRDS(paste0(here("data", "/"), "GSE75010_cell_type_proportions_pca_2022-05-03.rda"))
...
## Not adjusted for GA
```{r}
base.model.no.ga <- run_limma_model(d = d,
...
#cell.types.sig.kc.pr.only <- c("Maternal.Plasma.Cells", "Maternal.Natural.Killer.Cells", "Maternal.B.Cells", "Fetal.Naive.CD8..T.Cells", "Fetal.Mesenchymal.Stem.Cells", "Fetal.Memory.CD4..T.Cells", "Fetal.Hofbauer.Cells", "Fetal.Fibroblasts", "Fetal.Extravillous.Trophoblasts", "Fetal.Cytotrophoblasts")

cell.types.sig <- c("Maternal.Plasma.Cells", "Fetal.Mesenchymal.Stem.Cells", "Fetal.Hofbauer.Cells", "Fetal.Fibroblasts", "Fetal.Extravillous.Trophoblasts")
```
...
## PC3 Only
Reducing the cell composition adjustment to just PC3 does not completely attenuate DEX results.
```{r}
...
genes.of.interest <- c("FLT1", "LEP", "ENG")
...
p1.legend.plot <- p1 + theme(legend.text = element_text(size = 12))

# Get legend for shared legend
p1.legend <- get_legend(p1.legend.plot + guides(colour = guide_legend(override.aes = list(size=6), reverse = TRUE)))

p2 <- p2 + coord_cartesian(xlim = c(-.45, 1.2), ylim = c(0, 5)) + ggtitle("Cell type-adjusted")

# Arrange multipanel figure
panel <- ggarrange(p1, p2, ncol = 2, nrow = 1, labels = "auto", font.label = list(size = 24, face = "bold", color = "black"), common.legend = TRUE, legend.grob = p1.legend, legend = "bottom")
#print(panel)
...
```{r}
base.res.formated <- base.model$res %>%
  dplyr::select(gene_symbol, logFC, CI.L, CI.R, AveExpr, t, P.Value, adj.P.Val) %>%
  arrange(adj.P.Val, desc = T) %>%
  #mutate(across(c("estimate", "std.error", "statistic", "p.value", "conf.low", "conf.high"), ~round(., digits = 3)))
  mutate(across(c(logFC, CI.L, CI.R, AveExpr, t, P.Value, adj.P.Val), ~round(., digits = 3))) %>%
  mutate(P.Value = ifelse(P.Value == 0, "p<0.001", P.Value)) %>%
  mutate(adj.P.Val = ifelse(adj.P.Val == 0, "p<0.001", adj.P.Val))
colnames(base.res.formated) <- c("Gene", "Log2 Fold Change", "Lower 95% Confidence Interval", "Upper 95% Confidence Interval", "Average Expression", "Moderated t-statistic", "p-value", "False discovery adjusted p-value")

pc.res.formated <- pc.model$res %>%
  dplyr::select(gene_symbol, logFC, CI.L, CI.R, AveExpr, t, P.Value, adj.P.Val) %>%
  arrange(adj.P.Val, desc = T) %>%
  #mutate(across(c("estimate", "std.error", "statistic", "p.value", "conf.low", "conf.high"), ~round(., digits = 3)))
  mutate(across(c(logFC, CI.L, CI.R, AveExpr, t, P.Value, adj.P.Val), ~round(., digits = 3))) %>%
  mutate(P.Value = ifelse(P.Value == 0, "p<0.001", P.Value)) %>%
  mutate(adj.P.Val = ifelse(adj.P.Val == 0, "p<0.001", adj.P.Val))
colnames(pc.res.formated) <- c("Gene", "Log2 Fold Change", "Lower 95% Confidence Interval", "Upper 95% Confidence Interval", "Average Expression", "Moderated t-statistic", "p-value", "False discovery adjusted p-value")

list.for.export <- list(`Base Model` = base.res.formated, `Cell Type Adjusted Model` = pc.res.formated)

#write.xlsx(x = list.for.export, file = here("results", "GSE75010", paste0(Sys.Date(), "_GSE75010_dex_full_res.xlsx")))
```


Need to run this chunk in the console to function properly
```{r, eval = F}
png(filename = here("results", "GSE75010", paste0(Sys.Date(), "_cell_type_adj_compare_multipanel.png")), width = 2816, height = 2160, units = "px", res = 300)
panel
dev.off()
```
...
# Write  .rnk file
#write_tsv(gsea.input.pc, here("results", "GSE75010", paste0(Sys.Date(), "_GSEA_pc_preranked_input.rnk")), col_names = FALSE)
```
...
# Write  .rnk file
#write_tsv(gsea.input.base, here("results", "GSE75010", paste0(Sys.Date(), "_GSEA_base_preranked_input.rnk")), col_names = FALSE)
```

## Prep mediation data and explore correlation and interaction relationships
...
```{r}
interaction <- topTable(fit, coef = "Dim.1", number = Inf)
interaction <- rownames_to_column(interaction, var = "gene_symbol")
res.flt1 <- interaction %>%
  filter(gene_symbol == "FLT1")
print(res.flt1)

interaction <- topTable(fit, coef = "Dim.2", number = Inf)
interaction <- rownames_to_column(interaction, var = "gene_symbol")
res.flt1 <- interaction %>%
  filter(gene_symbol == "FLT1")
print(res.flt1)
