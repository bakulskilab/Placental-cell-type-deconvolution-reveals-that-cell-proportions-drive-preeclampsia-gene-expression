...
gene.sums <- data.ciber %>%
  rowwise() %>%
  mutate(gene.sum = sum(c(Cytotrophoblast, Endothelial, `Extravillous Trophoblast`, Fibroblast, `Hofbauer Cell`, Leukocytes, Syncytiotrophoblast)))
summary(gene.sums)
...
```{r}
testRes = cor.mtest(data.ciber, conf.level = 0.95)
```
