...
```{r}
merged <- readRDS(here("data", "GSE75010_with_abundances_2022-04-12.rda"))
```
...
active <- 
  merged %>%
  dplyr::select(!c(sample, phenotype, batch, ga, sex, "P-value", RMSE, Correlation, "Fetal Naive CD4+ T Cells", "Fetal B Cells"))
...
# The first 5 components provide almost 78% of the variance explained.
sum(prop.var[2,1:5])
```
...
```{r}
long.maternal %>% group_by(phenotype, Cell.Type) %>% summarise(median = median(Proportion))
```
...
merged %>%
  ...
  colSums()
# Exclude Fetal Naive CD4+ T Cells and Fetal B Cells; no longer necessary with the Tsang data integration
