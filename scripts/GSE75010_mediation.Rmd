...
```{r}
df <- readRDS(file = here("data", "2021-08-14_med_data.rda"))
df.all <- readRDS(file = here("data", "2021-08-17_all_med_data.rda"))
```

# g-formula Mediation in CMAVerse {.tabset}

Trying the CMAVerse approach for simplicity and to double-check proper specification of mediation models
```{r}
...
# Transcriptome-wide Mediation Analysis {.tabset}
```{r, results='hide'}
# Genes start at column 61
colnames(df.all[61:75])

#outcomes <- colnames(df.all[,61:75])

outcomes <- c("LEP", "IGF1", "FSTL3")
res <- outcomes %>%
  map(function(x)
    cmest(data = df.all, model = "gformula", outcome = x, exposure = "phenotype",
                      mediator = c("Dim.1", "Dim.2", "Dim.3", "Dim.4", "Dim.5"), basec = c("sex", "batch"), postc = "ga", EMint = TRUE,
                      mreg = list("linear", "linear", "linear", "linear", "linear"), yreg = "linear", postcreg = list("linear"),
                      astar = 0, a = 1, mval = list(0, 0, 0, 0, 0), estimation = "imputation", inference = "bootstrap", nboot = 2)
  )
```

## Imai Mediation Package Approach
