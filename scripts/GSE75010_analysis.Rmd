...
long <- gather(merged.violin, cell_type, proportion, fibroblasts:extravillous_trophoblasts, factor_key = F)
long.placenta <- long %>%
...
```{r}
ggplot(long.placenta, aes(x = cell_type, y = proportion, fill = phenotype)) + geom_violin(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) + theme(legend.position = c(.1, .8)) + theme(axis.text.x = element_text(angle = 45))
#ggsave(paste0(data_dir, "vln_placenta.png"), device = png())
```


```{r}
ggplot(long.immune, aes(x = cell_type, y = proportion, fill = phenotype)) + geom_violin(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) + theme(legend.position = c(.1, .8)) + theme(axis.text.x = element_text(angle = 45))
