...
volcanos <- map2(mapped, contrast.labels,
                 ~ .x[["volcano"]] + ggtitle(.y) + coord_cartesian(xlim = c(x_min, x_max), ylim = c(y_min, y_max)) + theme_bw(base_size = 20) + theme(legend.position = "none") + ylab("-log10 q-value"))
...
```{r}
panel <- ggarrange(volcanos[[1]], volcanos[[2]], volcanos[[3]], volcanos[[4]], volcanos[[5]], volcanos[[6]], volcanos[[7]], ncol = 4, nrow = 2, labels = "AUTO", font.label = list(size = 24, face = "bold", color = "black"))
#print(panel)
...
ct.res <- results$cell.typecytotrophoblast
...
cell.type.labels <- c("Composite", "Cytotrophoblast" , "Endothelial", "Extravillous Trophoblast", "Fibroblast", "Hofbauer Cell", "Leukocyte", "Syncytiotrophoblast")
...
#ggsave(here("results", "bulk", paste0(Sys.Date(), "_sorted_PCA.png")))
```