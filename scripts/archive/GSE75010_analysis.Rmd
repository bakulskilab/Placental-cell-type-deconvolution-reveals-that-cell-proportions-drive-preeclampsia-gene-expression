...
levels(long$phenotype) <- c("Control", "Preeclampsia Case")
...
```{r}
pl <- ggplot(long.placenta, aes(x = proportion, y = cell_type, color = phenotype, point_color = phenotype, fill = phenotype)) +
  geom_density_ridges(
...
#ggsave(paste0(data_dir, "cellpropsplacenta.png"))
pl

im <- ggplot(long.immune, aes(x = proportion, y = cell_type, color = phenotype, point_color = phenotype, fill = phenotype)) +
  geom_density_ridges(
    jittered_points = TRUE, scale = .95, rel_min_height = .01,
    point_shape = "|", point_size = 1, size = 0.25,
    position = position_points_jitter(height = 0)
  ...
  theme_ridges(center = TRUE) +
  theme(legend.title = element_blank(), legend.position = "none", legend.text = element_text(size = 24))
#ggsave(paste0(data_dir, "cellpropsimmune.png"))

im

figure <- ggarrange(pl, im,
                    ncol = 1, nrow = 2)

#ggsave(paste0(data_dir, "cellprops.png"))
```


```{r}
p1 <- ggplot(long.placenta, aes(x = cell_type, y = proportion, fill = phenotype)) + geom_density_ridges() + theme(axis.text.x = element_text(angle = 45)) + theme(legend.title = element_blank(), legend.position = c(.15, .85)) +  ggtitle("Estimated Placental Cell Type Proportions") + xlab("Cell Type") + ylab("Cell Type Proportion") + scale_y_continuous(labels = scales::percent_format(accuracy = 1))
```


```{r}
p1 <- ggplot(long.placenta, aes(x = cell_type, y = proportion, fill = phenotype)) + geom_violin(scale = "width", draw_quantiles = c(0.25, 0.5, 0.75)) + theme(axis.text.x = element_text(angle = 45)) + theme(legend.title = element_blank(), legend.position = c(.15, .85)) +  ggtitle("Estimated Placental Cell Type Proportions") + xlab("Cell Type") + ylab("Cell Type Proportion") + scale_y_continuous(labels = scales::percent_format(accuracy = 1)) + coord_flip()
p1
...
```{r}
forest <- ggplot(data=estimates, 
                       aes(x=cell_type, y=or, ymin=lower, ymax=upper)) +
