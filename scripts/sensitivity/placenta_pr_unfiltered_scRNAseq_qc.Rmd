---
title: "Pique-Regi TNL scRNA-seq import and QC of placental villous tissue"
...
tnl.478 <- tenx_to_seu(counts.matrix = readRDS(paste0(data_dir, "tnl_478_raw.rda")),
                      project.name = "tnl.478")
tnl.481 <- tenx_to_seu(counts.matrix = readRDS(paste0(data_dir, "tnl_481_raw.rda")),
                      project.name = "tnl.481")
tnl.484 <- tenx_to_seu(counts.matrix = readRDS(paste0(data_dir, "tnl_484_raw.rda")),
                      project.name = "tnl.484")
# Merge
tnl <- merge(tnl.478, y = c(tnl.481, tnl.484), add.cell.ids = c("tnl.478", "tnl.481", "tnl.484"), project = "tnl")
# Clean-up memory
rm(tnl.478, tnl.481, tnl.484)
```

```{r}
# Convert to SingleCellExperiment for QC
sce <- as.SingleCellExperiment(tnl)
# Clean-up memory
rm(tnl)
```
...
```{r}
tnl <- as.Seurat(filtered)
#saveRDS(tnl, paste0(data_dir, "placenta_scRNA_seq_pr_filtered_", Sys.Date(), ".rda"))
```
