...
Load dex results
```{r}
markers <- readRDS(paste0(data_dir,"all_markers_cleaned_combined_seurat_2020-12-22.rda"))
```
...
```{r}
# All results
#write.csv(markers, paste0(results_dir, "all_markers_cleaned_combined_seurat_2020-12-22.csv"))
...
top.10.per <- markers %>%
  ...
  slice_head(n=10)
# Top 10 per cluster
#write.csv(top.10.per, paste0(results_dir, "top_10_overexpressed_q0.05_markers_cleaned_combined_seurat_2020-12-22.csv"))
```
...
map2(.x = top,                  # List of tibbles containing dex results
     ...
     output_dir = paste0(results_dir, "markerVolcanos/") # Save directory
     )
```
...
```{r}
gostres <- readRDS(paste0(results_dir, "seurat_all_markers_gost_results_2021-01-05.rda"))
...
#saveRDS(gostres, paste0(results_dir, "seurat_all_markers_gost_results_", Sys.Date(), ".rda"))
...
write.csv(gostres.table, paste0(results_dir, "seurat_all_markers_gost_results_table_", Sys.Date(), ".csv"))
...
#write.csv(gostres.small.table, paste0(results_dir, "seurat_all_markers_gost_results_table_top10_BP_filtered_", Sys.Date(), ".csv"))
```
...
for(idx in 1:length(cluster.names)) {
  ...
  #ggsave(paste0(data_dir, "gProfiler/sc_clusters/", cluster.names[idx], "_gost_plot_200_term_size_", Sys.Date(), ".png"), device = "png")
}
```
