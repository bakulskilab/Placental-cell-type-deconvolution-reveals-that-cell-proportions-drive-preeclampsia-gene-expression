...
ggplot(cor.df, aes(x = correlation)) +
  ...
  labs(title = "Sample 1 Cluster Averages Technical Replication", y = element_blank(), x = "Spearman Correlation") +
  coord_flip() +
...
ggplot(cor.df, aes(x = correlation)) +
  geom_dotplot(binwidth = .01) +
  labs(title = "Sample 2 Cluster Averages Technical Replication", y = element_blank(), x = "Spearman Correlation") +
  coord_flip() +
  geom_text_repel(aes(y = .001, x = correlation, label=ifelse(correlation < .9, cell.type, '')), label.padding = 5, force = 2) 
```
