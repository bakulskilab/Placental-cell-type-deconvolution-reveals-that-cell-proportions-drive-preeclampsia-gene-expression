...
run_limma_model <-
  function(d,
    ...
    hits <- topTable(fit, coef = "phenotypePreeclampsia", number = Inf)
...
p1.legend <- get_legend(p1 + guides(colour = guide_legend(override.aes = list(size=8), reverse = TRUE)))
...
# Bootstrap CIs
med.out.boot <- mediate(med.fit, out.fit, treat = "phenotype", mediator = "Dim.3", boot = TRUE, boot.ci.type = "bca", sims = 2000)
...
med.out.boot <- mediate(model.m, model.y, treat = "phenotype", mediator = "Dim.3", boot = TRUE, boot.ci.type = "bca", sims = 2000)
summary(med.out.boot)
