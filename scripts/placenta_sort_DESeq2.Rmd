...
print(paste0(
  length(rownames(dds)),
  " total gene features assessed with featureCounts"
))
...
genes <- gconvert(query = rownames(dds), organism = "hsapiens",
target="HGNC", mthreshold = Inf, filter_na = FALSE)
```
...
```{r}
genes %>% group_by(target) %>% tally() %>% filter(n > 1)
```

Check for duplicate Ensembl IDs
```{r}
genes %>% group_by(input) %>% tally() %>% filter(n>1)
```
...
summarizeDESeq2Results <-
   function(DESeq2_Results, Cell_Type_Identity, FDR_Threshold, Genes_Anno_Tbl) {
     ...
     print(
       paste0(
         "Returning differentially expressed results at FDR-adjusted ",
         FDR_Threshold
       )
     )
...
```{r}
dds$cell.type %>% levels
```
Rename levels to two character factor levels for easier coding, doesn't really help
```{r, eval = F}
dds$cell.type <- 
  recode(
    dds$cell.type,
...
levels(dds$cell.type)

#dds$cell.type <-
#  factor(dds$cell.type,
#    labels = c(
#      "Composite",
#      "Cytotrophoblast",
#      "Endothelial",
...
results <- results(
  dds,
  contrast = list( "cell.typecytotrophoblast", 
...
run_deseq2 <- function(target) {
  ...
  # Return a list doesn't work with lapply/map
...
mapped <- lapply(cell.types, run_deseq2)
