...
cluster.ids <-
  WhichCells(object = seuratObject, idents = clusterIdentity)
cluster.sc <- subset(x = seuratObject, cells = cluster.ids)

# Calculate Average Expression for cell type
cluster.expr <- AverageExpression(cluster.sc) %>% as.data.frame()
...
cluster.sc.gex <-
  dplyr::select(cluster.sc.gex, ensembl_gene_id, everything())
