...
```{r}
plot1 <- base.model$volcano
plot1

plot2 <- pc.model$volcano
plot2 + ggtitle("Cell type-adjusted")
```
...
Draw and save.
```{r, eval = FALSE}
png(paste0(here("results", "GSE75010", "/"), "GSE75010_dex_2_panel_", Sys.Date(), ".png"))
grid.newpage()
...
gsea.input.pc <- pc.res %>% select(gene_symbol, t) %>% arrange(desc(t))
...
```{r}
# Extract model cell type-adjusted model results relevant to GSEA
gsea.input.base <- base.res %>% select(gene_symbol, t) %>% arrange(desc(t))
# Check for duplicated t-statistics, there are none
